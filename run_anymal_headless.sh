...
prepare_training_run "$VARIANT" headless
unset R2DREAMER_INIT_CHECKPOINT || true
export R2DREAMER_SAVE_GIFS="${SAVE_GIFS:-0}" R2DREAMER_GIF_FPS="${GIF_FPS:-8}" R2DREAMER_GIF_SCALE="${GIF_SCALE:-2}" R2DREAMER_GIF_MAX_BATCH="${GIF_MAX_BATCH:-1}" R2DREAMER_GIF_NAMES="${GIF_NAMES:-train_video,open_loop}"
cmd=(
  "$ISAACLAB_SH" -p "$BUNDLE_DIR/r2dreamer_train.py" --num_envs "$NUM_ENVS" --headless --enable_cameras
  ...
  "batch_size=$BATCH_SIZE" "batch_length=$BATCH_LENGTH" buffer.storage_device=cpu buffer.max_size=500000
  trainer.video_pred_log=true "logdir=$LOGDIR" "$@"
)
initialize_training_run "$VARIANT" headless "run_anymal_headless.sh" "${cmd[@]}"
