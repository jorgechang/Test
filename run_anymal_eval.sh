...
if [[ -z "${OUTPUT_DIR:-}" ]]; then OUTPUT_DIR="$(default_eval_output_dir "$CHECKPOINT_PATH")"; fi
...
cd "$ISAACLAB_ROOT"
exec "$ISAACLAB_SH" -p "$BUNDLE_DIR/r2dreamer_eval.py" --checkpoint "$CHECKPOINT_PATH" --episodes "$EPISODES" --num_envs "$NUM_ENVS" --video_steps "$VIDEO_STEPS" --headless --enable_cameras "${extra[@]}" \
